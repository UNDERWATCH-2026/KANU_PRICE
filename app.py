    ...
    if intent == "DISCOUNT":
        ...
        for _, row in df.iterrows():
            ...
            if url in discount_map:
                ...
                # 할인 기간 조회 - raw_daily_prices에서 직접 계산
                def calc_discount_periods(product_url, dfrom, dto):
                    r = (
                        supabase.table("raw_daily_prices")
                        .select("date, normal_price, sale_price")
                        .eq("product_url", product_url)
                    ...
                    for rd in r.data:
                        ...
                        if is_disc:
                            ...
                            if start is not None:
                                periods.append({"discount_start_date": start, "discount_end_date": prev_date})
                                start = None
                    if start is not None:
                        periods.append({"discount_start_date": start, "discount_end_date": prev_date})
                    return periods

                period_res_data = calc_discount_periods(
                    row["product_url"],
                    date_from_str,
                    date_to_str
                )
...
| 할인 제품 조회 | 할인 제품 |
| 할인 기간 | 할인 기간, 언제 할인 |
| 할인율 분석 | 할인율 가장 큰/높은/낮은, 최대/최소 |
| 할인가 변동 | 할인가 인상/인하, 판매가 상승/하락 |
| 정상가 변동 | 정상가 변동, 가격 인상/인하 |
| 최저가·최고가 | 최저가, 가장 싼, 최고가, 가장 비싼 |
| 할인 중 최저가 | 할인 제품 중 최저가 |
| 품절 제품 | 품절 제품, 품절된 |
| 복원 제품 | 복원, 재입고 |
| 신제품 | 신제품, 신상, 출시, 새로 나온 |
